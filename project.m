...
% import the background project and show it on the axes
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    input = getappdata(0, 'a');
    result = getappdata(0, 'result');
    % Call histogramMatching function
    outputImage = histogramMatching(input, result);
    % Draw histogram for the modified project
    outputHistogram = drawHistogram(outputImage);
    % Set the 'filename' appdata with the modified project
    setappdata(0, 'filename', outputImage);
    % Display the modified project in axes
    axes(handles.axes2);
    imshow(outputImage);
    % Display the histogram of the modified project in another axes
    axes(handles.axes3);
...
% hObject    handle to size_max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of size_max as text
%        str2double(get(hObject,'String')) returns contents of size_max as a double
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
x1 = get(handles.wi, 'string');
x1 = str2num(x1);
a=getappdata(0,'a');
input=a;
input=weightfilter(input,x1);
input1=drawHistogram(input);
setappdata(0,'filename',input1);
setappdata(0,'filename',input);
axes(handles.axes11);
