function imageBorder = SmoothingWithMeanFilter(oldImage, sizeMask)
    [row, col, ch] = size(oldImage);
    paddingImage = zeros(row, col, ch);
    boundary = floor(sizeMask / 2);%determine padding boundry
    ...
    for i = 1:row
        for j = 1:col
            for k = 1:ch
                ...
                filteredImage(i, j, k) = mean(localRegion(:));
            end
        end
    end
